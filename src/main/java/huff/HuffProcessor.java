...
public class HuffProcessor implements Processor {
    ...
    private HuffNode makeTreeFromCounts(int[] array) {
        ...
        while (pq.size() > 1) {
            HuffNode child1 = pq.poll();
            HuffNode child2 = pq.poll();
            HuffNode parent = new HuffNode(-1, child1.weight() + child2.weight(), child1, child2);
            pq.add(parent);
        }
    ...
    private void makeCodingsFromTree(HuffNode hf, String[] encodings, String encoding) {
        if (hf == null){
            return;
        }
        if (hf.isLeaf() == true)
            encodings[hf.value()] = encoding;
        else {
            makeCodingsFromTree(hf.left(), encodings, encoding + "0");
            makeCodingsFromTree(hf.right(), encodings, encoding + "1");
        }
    }
        /** Debugging method.
    ...
    private void writeCompressedBits(BitInputStream in, String[] codings, BitOutputStream out) {
        ...
        while (bitsRead != -1) {
            bitsRead = in.read();
            if (bitsRead != -1) {
                String strCode = codings[bitsRead];
                int code = Integer.parseInt(strCode, 2);
                out.writeBits(strCode.length(), code);
            }
        }

        out.writeBits(8, PSEUDO_EOF);
    }
